...
const supportWhatsAppHref = "https://wa.me/529381573988?text=Hola%2C%20quiero%20m%C3%A1s%20informaci%C3%B3n%20sobre%20AionSite%20Shop."

export default async function PublicLayout({ children }: { children: React.ReactNode }) {
  const session = await auth()
  const dashboardMembership = session?.user?.id
  ...
  return (
    <SessionProvider session={session}>
      <Navbar session={session} dashboardSlug={dashboardMembership?.store.slug ?? null} />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
            <div className="col-span-2 md:col-span-1">
              ...
              <div className="flex items-center gap-3">
                <a href="https://www.facebook.com/shopaionsite" target="_blank" rel="noopener noreferrer" className="h-9 w-9 rounded-full bg-muted/50 flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-colors" aria-label="Facebook">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
                <a href="https://www.instagram.com/shopaionsite/" target="_blank" rel="noopener noreferrer" className="h-9 w-9 rounded-full bg-muted/50 flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-colors" aria-label="Instagram">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <a href={supportWhatsAppHref} target="_blank" rel="noopener noreferrer" className="h-9 w-9 rounded-full bg-muted/50 flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-colors" aria-label="WhatsApp para más información">
