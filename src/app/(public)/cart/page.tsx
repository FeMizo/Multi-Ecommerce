...
export default function CartPage() {
  ...
  const checkoutStoreName = itemGroups[0]?.storeName ?? null
            ...
            <div key={group.storeId} className="space-y-3">
              <div className="flex items-center justify-between rounded-xl border bg-muted/30 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{group.items.length} producto(s)</p>
                </div>
                <p className="text-sm font-medium">
                  {formatPrice(group.items.reduce((sum, item) => sum + item.price * item.quantity, 0))}
                </p>
              </div>
                ...
                <div key={item.id} className="flex gap-4 p-4 rounded-xl border bg-card">
                  <div className="relative h-20 w-20 rounded-lg overflow-hidden bg-muted shrink-0">
                    <Image
                      ...
                      alt={item.image ? item.name : `Imagen generica de ${item.name}`}
                      fill
